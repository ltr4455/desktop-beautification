...
- ✅ **v0.1 功能全部完成**；**35 个单元测试全部通过**（`npm test`，6 个测试文件）。
...
- ✅ **新增（2026-08-01）**：**全屏应用/游戏自动隐藏悬浮层**（设置 → 常规开关，默认开启）：800ms 轮询前台窗口（koffi GetForegroundWindow/GetMonitorInfoA），全屏/无边框全屏时自动隐藏，回到桌面恢复；悬停操作有 4 秒宽限期；原生层不可用时自动降级。决策逻辑抽为纯函数 `src/main/fullscreen.js` 并可单测。
- ✅ **新增（2026-08-01）**：**交互门卫**——悬浮层仅在鼠标位置的最顶层窗口是它自己时才响应（WindowFromPoint + GetAncestor），上层普通/透明/穿透窗口不会再让鼠标事件漏到悬浮层触发误操作。
- ✅ **新增（2026-08-01）**：**应用窗口聚焦时的悬浮层策略（区分全屏/非全屏）**——全屏应用/无边框全屏游戏 → 悬浮层隐藏；非全屏应用窗口 → 悬浮层保持显示但不响应（点击穿透 + `body.inert`，无悬停/动效/提示/菜单）；桌面焦点 → 正常响应。状态机 active/inert/hidden，决策纯函数 `src/main/overlay.js`，可单测。
