...
- ✅ **新增（2026-08-01）**：**应用窗口聚焦时的悬浮层策略**——全屏应用/无边框全屏游戏 → 悬浮层隐藏；非全屏应用窗口 → 悬浮层保持显示，鼠标落在该应用窗口内时不响应（位置判定：GetCursorPos + GetWindowRect 对比前台窗口矩形），鼠标在桌面区域时正常响应。决策纯函数 `src/main/overlay.js`，可单测。
